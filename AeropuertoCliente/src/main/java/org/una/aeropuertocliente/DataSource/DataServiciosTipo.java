 ...
 * @author Daniel
 */
public class DataServiciosTipo implements JRDataSource{
    ...
    float num=1000;
    int cont=0;
    public DataServiciosTipo() throws InterruptedException, ExecutionException, IOException {
        index=-1;
        token = FlowController.getInstance().authenticationResponse.getJwt();
        list = ServicioWebService.getServicioByTipoServicioId(FlowController.getInstance().idTipoServicio, token);
    ...
    public Object getFieldValue(JRField jrf) throws JRException {
        ...
        switch(fieldName){
        case "Precio":
            value = precios.get(precios.size()-1).getMonto()+"";
            break;
        case "Factura":
        ...
        case "EstadoCobro":
            ...
            break;
        case "TipoServicio":
            value = list.get(0).getTipoServicio().getNombre()+"";
            break;
        case "Nombre":
